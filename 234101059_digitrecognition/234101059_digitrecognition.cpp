// 234101059_digitrecognition.cpp : Defines the entry point for the console application.
//
...
void train_model(int digit, int utterance){
	...
	FILE *fp;
	char filenameA[40];
	_snprintf(filenameA,40,"234101059_lambda/A_%d_%d.txt",digit,utterance);
	...
	//Store B in file
	char filenameB[40];
	_snprintf(filenameB,40,"234101059_lambda/B_%d_%d.txt",digit,utterance);
...
void calculate_avg_model_param(int d){
	...
	for(int u=1;u<=20;u++){
		char filenameA[40];
		_snprintf(filenameA,40,"234101059_lambda/A_%d_%d.txt",d,u);
		fp=fopen(filenameA,"r");
		for (int i = 1; i <= N; i++)
		{
			for (int j = 1; j <= N; j++)
		...
		fclose(fp);
		char filenameB[40];
		_snprintf(filenameB,40,"234101059_lambda/B_%d_%d.txt",d,u);
		fp=fopen(filenameB,"r");
		for (int i = 1; i <= N; i++)
		{
			for (int j = 1; j <= M; j++)
...
void store_final_lambda(int digit){
	FILE *fp;
	char filenameA[40];
	_snprintf(filenameA,40,"234101059_lambda/A_%d.txt",digit);
	fp=fopen(filenameA,"w");
	for (int i = 1; i <= N; i++)
	{
		for (int j = 1; j <= N; j++)
	...
	fclose(fp);
	char filenameB[40];
	_snprintf(filenameB,40,"234101059_lambda/B_%d.txt",digit);
	fp=fopen(filenameB,"w");
	for (int i = 1; i <= N; i++)
	{
		for (int j = 1; j <= M; j++)
...
void processTestFile(int d){
	...
	initialization();
	char filenameA[40];
	_snprintf(filenameA,40,"234101059_lambda/A_%d.txt",d);
	fp=fopen(filenameA,"r");
	if (fp == NULL){
		printf("Error\n");
	}
	...
	char filenameB[40];
	_snprintf(filenameB,40,"234101059_lambda/B_%d.txt",d);
	fp=fopen(filenameB,"r");
	for (int i = 1; i <= N; i++){
		for (int j = 1; j <= M; j++){
			fscanf(fp, "%Lf ", &B[i][j]);
...
//Training HMM for given dataset
void training(){
	...
	for(int d=0;d<=9;d++){
			printf("Training started for digit %d:\n\n", d);
			for(int u=1;u<=TRAIN_SIZE;u++){
				char filename[40];
				_snprintf(filename,40,"234101059_dataset/234101059_E_%d_%d.txt",d,u);
		...
		printf("Training completed for digit %d\n\n", d);
		printf("\n================================\n\n");
	}
}

//Testing HMM for given dataset
void testing(){
	double accuracy=0.0;
	for(int d=0;d<=9;d++){
		printf("Testing started for digit %d:\n", d);
		for(int u=TRAIN_SIZE+1;u<=TEST_SIZE;u++){
			char filename[40];
			_snprintf(filename,40,"234101059_dataset/234101059_E_%d_%d.txt",d,u);
			generate_observation_sequence(filename);
			printf("\nUtterance: %d", u);
...
//Live testing of Digit Recognition
void live_test_HMM(){
	Sleep(2000);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	while(true)
	{
		int choice;
		printf("\n\n***********************************************************");
		printf("\n1. Do testing on pre-recorded files");
		printf("\n2. Do live testing");
		printf("\n3. Exit\n");
		printf("\nEnter your choice: ");
		scanf("%d",&choice);
